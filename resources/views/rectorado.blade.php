        ...
        <div class="col-md-12">
            <nav>
                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                    <a class="nav-item nav-link  @if(!isset($flag)) active @endif" id="nav-follow-tab" data-toggle="tab" href="#nav-follow" role="tab" aria-controls="nav-follow" aria-selected="false">Expedientes Seguidos</a>

                    <a class="nav-item nav-link" id="nav-home-tab" data-toggle="tab" href="#nav-rect" role="tab" aria-controls="nav-rect" aria-selected="false">Expedientes En Rectorado</a>
            ...
            <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane fade  @if(!isset($flag)) show active @endif" id="nav-follow" role="tabpanel" aria-labelledby="nav-follow-tab">
            <div class="card">
                ...
                <div class="card-body">
                        ...
                        <table class="table table-sm table-striped" id="tabla_1">
                            ...
                            <tbody>
                                @foreach ($followed as $item)
                                    ...
                                    @if($item->getExpediente->getPases->last()->codigo_destino == 983)
                                    {{-- FILTRO AQUELLOS EXPEDIENTE QUE SU ULTIMO PASE SE RECTORADO --}}
                                    <tr>
                                    <td scope="row" style="width:8rem;">{{$item->getExpediente->numero}}</td>
                                    {{-- <td scope="row">{{$item->getExpediente->hash()}}</td> --}}
                                       @if($item->getExpediente->getAsunto)

                                    <td scope="row" colspan="1">{{$item->getExpediente->getAsunto->descripcion}}</td>
                                    @else
                                    <td></td>
                                    @endif
                                    <td scope="row">{{$item->getExpediente->detalle_asunto}}</td>
                                    {{-- <td>{{$item->getExpediente->getPases->last()->destino}}</td> --}}
                                    <td style="width:6rem;">{{$item->getExpediente->getPases->last()->fecha}}</td>
                                    {{-- <td style="width:6rem;">{{$item->getExpediente->getPases->last()->fecha_ingreso}}</td>
                                    <td style="width:6rem;">{{$item->getExpediente->getPases->last()->fecha_salida}}</td> --}}
                                    @if($item->getExpediente->getEstado() == "no-recibido")
                                        <td  scope="row" class="bg-danger" style="width:6rem;">No Recibido</td>
                                    @elseif($item->getExpediente->getEstado() == "recibido")
                                        <td  scope="row" class="bg-warning" style="width:6rem;">Recibido</td>
                                    @elseif($item->getExpediente->getEstado() == "tratandose")
                                        <td scope="row"  class="bg-success" style="width:6rem;">Tratándose</td>
                                    @endif


                                    <td scope="row" style="width:6rem;">{{$item->getExpediente->diff}} días</td>
                                    <td scope="row" class="excludeThisClass"><a name="" id="" class="btn btn-sm btn-primary btn-block" href="{{ route('movimientos', ['exp' => $item->getExpediente->numero]) }}" role="button">Ver Movimientos <span class="badge badge-light">{{$item->getExpediente->getPases->count()}}</span> </a></td>
                                    @if($item->getExpediente->seguido())
                    ...
                    {{-- TAB FOLLOW --}}
                    <div class="tab-pane fade" id="nav-rect" role="tabpanel" aria-labelledby="nav-home-tab">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-10">
                        ...
                        <table class="table table-sm table-striped" id="tabla_1">
                            ...
                            <tbody>
                                @foreach ($expedientes as $item)
                                    ...
                                    @if($item->getPases->last()->codigo_destino == 983 && !$item->seguido())
                                    {{-- FILTRO AQUELLOS EXPEDIENTE QUE SU ULTIMO PASE SE RECTORADO --}}
                                    <tr>
